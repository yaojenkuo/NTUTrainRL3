...
Dependent variable is Poisson distributed(count data).

```{r}
lmPoisson <- glm(breaks ~ tension, data=warpbreaks, family="poisson")
summary(lmPoisson)
```

## Binomial

Dependent variable is binomially distributed(0 v.s. 1 data).

```{r}
lmBinomial <- glm(vs ~ hp+mpg+gear,data=mtcars, family=binomial)
summary(lmBinomial)
```
